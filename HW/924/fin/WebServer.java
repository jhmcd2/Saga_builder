...
import gov.ic.silkwave.registration.QueuesAdminServlet;
...
public class WebServer {
    ...
    private WebServer() {
        try {
            ...
            if (httpEnabled || httpsEnabled) {
                ...
                context.setBaseResourceAsString(webcontent);
                ...
                context.addServlet(new ServletHolder(new QueuesAdminServlet()), "/QueuesAdminServlet/*");
                context.addServlet(new ServletHolder(new HealthServlet()), "/health");
                context.addServlet(new ServletHolder(new PropertiesServlet()), "/properties");
                ...
                context.addServlet(exportHolder, "/resources-export");
                ServletHolder resourcesDocsHolder = new ServletHolder("resources-docs", DefaultServlet.class);
                resourcesDocsHolder.setInitParameter("resourceBase",
                    new File(webcontent, "resources-docs").getAbsolutePath());
                resourcesDocsHolder.setInitParameter("pathInfoOnly", "true");
                context.addServlet(resourcesDocsHolder, "/resources-docs/*");
                ...
                if (!casportSimEnabled) {
                    ...
                    handlers.setHandlers(context.get(), new DefaultHandler());
                } else {
                    ...
                    handlers.setHandlers(context.get(), casportSimContext.get(), new DefaultHandler());
                }
                ...
                if (httpsEnabled) {
                    ...
                    constraintMapping.setConstraint(allow);
                    constraintMapping.setPathSpec("/*");
                    securityHandler.addConstraintMapping(constraintMapping);


                    // Public constraints
                    ServletConstraint publicConstraint = ConstraintSecurityHandler.createConstraint("public", false, null, 0);
                    ...
                    constraintMapping = new ConstraintMapping();
                    constraintMapping.setConstraint(publicConstraint);
                    constraintMapping.setPathSpec("/resources-docs");
                    securityHandler.addConstraintMapping(constraintMapping);
                    ...
                    ServletConstraint constraint = ConstraintSecurityHandler.createConstraint("none", false, null, 0);

                    ServletConstraint none = ConstraintSecurityHandler.createConstraint("none", false, null, 0);
                    constraintMapping = new ConstraintMapping();
                    constraintMapping.setConstraint(none);
                    ...
                    if (realm != null) {
                        loginService.setConfig(ResourceFactory.of(loginService).newResource(realm));
                    } else {
            ...
            try {
                ...
                for (Object object : objects) {
                    if (object instanceof PingResponse response) {
                        builder.append(DateFromXMLAdapter.convert(Instant.now().toEpochMilli())).append(" ")
                        ...
                        if (OutcomeEnum.FAILURE == response.getOutcome()) {
                            String error = DateFromXMLAdapter.convert(
                                    Instant.now().toEpochMilli()) + " TraceRoute failed";
                ...
                builder.append(DateFromXMLAdapter.convert(Instant.now().toEpochMilli())).append(" Bad response from ")
                        .append(envelope.getSource()).append("\n");
            }
        })) {
            builder.append(DateFromXMLAdapter.convert(Instant.now().toEpochMilli())).append(" Trace Route Success.\n");
        } else {
            builder.append(DateFromXMLAdapter.convert(Instant.now().toEpochMilli())).append(" Trace Route Failed.\n");
        }
    ...
    private void disableTrackTrace(ConstraintSecurityHandler securityHandler) {
        ...
        enableEverythingButTraceTrackMapping.setConstraint(none);
    ...
    private ConstraintMapping setConstraintMapping(String constraintName, String methodName) {
        ServletConstraint constraint = ConstraintSecurityHandler.createConstraint(constraintName, true, null, 0);
