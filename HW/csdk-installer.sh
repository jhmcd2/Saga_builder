...
images_menu() {
  local images=(
    "pycs/pycs-client:2.10.1"
    "pycs/pycs-generate:4.0.3"
    "cs/csbuild:rh9min-jdk17-mvn3.9.6-cscli0.23.3"
    "cs/cs-cli:0.11.3"
    "cs/csta:1.3.1"
    "cs/aoco-cs:1.2.7"
    "silkwave-ir:3.2.4"
  ...
  while true; do
    ...
    if [[ -f $tar_file ]]; then
      ...
      if [[ ! -d $target_dir ]]; then
        echo "Image repository directory '$target_dir' does not exist. Extracting '$tar_file'..."
        mkdir "$target_dir"
        tar -xvzf "$tar_file" -C "$target_dir"
        ...
        echo "Image repository directory '$target_dir' already exists. No action taken."
      fi
    else
      echo "Tar file '$tar_file' does not exist. Please retrieve the '$tar_file' from bargin bin"
    fi    
    ###########################################################################
